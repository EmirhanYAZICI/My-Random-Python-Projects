...
def tel_no_sil(x):
    while True:
        ...
        silinecek_kisi= input('Lütfen silinecek kişinin adını yazınız: (çıkış için q): ').title()
        ...
        if silinecek_kisi in x:
            print(f'Bulunulan kayıt: {silinecek_kisi} - {x[silinecek_kisi]}')
...
# girdilere göre işlemler gerçekleştirilir ve sonuçlar ekrana yazdırılır. VE geliştirme için hata kontrolü ve veri doğrulama eklenebilir. ve acık eklemeler yapacağım sonradan.bu uygulam sıfırdan yazılacak ve time imoport edilecek ve aynı zamanda silme fonksiyonu düzeltilecek
